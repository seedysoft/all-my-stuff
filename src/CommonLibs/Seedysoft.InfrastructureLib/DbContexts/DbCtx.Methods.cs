...
{
    public async Task<Seedysoft.CoreLib.Entities.Pvpc[]> GetPvpcBetweenDatesAsync(DateTime sinceDate, DateTime untilDate, CancellationToken cancellationToken)
    {
        return await PvpcsView.AsNoTracking()
            ...
            .Select(x => (Seedysoft.CoreLib.Entities.Pvpc)x)
            .ToArrayAsync(cancellationToken)
            ?? Array.Empty<Seedysoft.CoreLib.Entities.Pvpc>();
    }
