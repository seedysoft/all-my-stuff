...
public class RoutesService(IConfiguration configuration, ILogger<RoutesService> logger) : GeocodingServiceBase(configuration)
{
    /// <summary>
    ...
    {
        Settings.RouteApi api = GeocodingSettings.RouteSettings.RouteApis.First(x => x.RouteName == GeocodingSettings.RouteSettings.CurrentImplementation);

        RouterBase router = GeocodingSettings.RouteSettings.CurrentImplementation switch
        {
...
#pragma warning restore format
            ...
            _ => throw new InvalidOperationException($"Unsupported router: {GeocodingSettings.RouteSettings.CurrentImplementation}"),
        };

        if (logger.IsEnabled(LogLevel.Information))
            logger.LogInformation("Using router implementation: {RouterImplementation}", GeocodingSettings.RouteSettings.CurrentImplementation);
