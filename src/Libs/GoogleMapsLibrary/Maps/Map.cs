...
public class Map : MVCObject, IJsObjectRef, IAsyncDisposable
{
        ...
        MapOptions? opts = null)
    {
        if (opts?.ApiLoadOptions != null)
        {
            ...
            await jsRuntime.InvokeVoidAsync("blazorGoogleMaps.objectManager.initMap", apiOpts);
        }

        JsObjectRef jsObjectRef = await JsObjectRef.CreateAsync(jsRuntime, "google.maps.Map", mapDiv, opts);
        JsObjectRef dataObjectRef = await jsObjectRef.GetObjectReference("data");
    ...
    public async Task AddControl(ControlPosition position, ElementReference reference) => _ = await _jsObjectRef.JSRuntime.MyInvokeAsync<object>("blazorGoogleMaps.objectManager.addControls", Guid.ToString(), position, reference);

    public async Task RemoveControl(ControlPosition position, ElementReference reference) => _ = await _jsObjectRef.JSRuntime.MyInvokeAsync<object>("blazorGoogleMaps.objectManager.removeControl", Guid.ToString(), position, reference);

    public async Task RemoveControls(ControlPosition position) => _ = await _jsObjectRef.JSRuntime.MyInvokeAsync<object>("blazorGoogleMaps.objectManager.removeControls", Guid.ToString(), position);

    public async Task AddImageLayer(ImageMapType reference) => _ = await _jsObjectRef.JSRuntime.MyInvokeAsync<object>("blazorGoogleMaps.objectManager.addImageLayer", Guid.ToString(), reference.Guid.ToString());
    public async Task RemoveImageLayer(ImageMapType reference) => _ = await _jsObjectRef.JSRuntime.MyInvokeAsync<object>("blazorGoogleMaps.objectManager.removeImageLayer", Guid.ToString(), reference.Guid.ToString());
    public async Task RemoveAllImageLayers() => _ = await _jsObjectRef.JSRuntime.MyInvokeAsync<object>("blazorGoogleMaps.objectManager.removeAllImageLayers", Guid.ToString());
    ...
    /// <returns></returns>
    public Task FitBounds(LatLngBoundsLiteral? bounds, OneOf<int, Padding>? padding = null) => _jsObjectRef.InvokeAsync("fitBounds", bounds, padding);
    ...
    public async Task<MapTypeId> GetMapTypeId()
    {
        ...
        return Helpers.Helper.ToEnum<MapTypeId>(mapTypeIdStr);
    }
