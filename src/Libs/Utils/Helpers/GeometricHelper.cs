...
public static class GeometricHelper
{
    ...
    public static double GetEarthRadiusKilometers(double latitudeInRadians)
    {
    ...
    public static double ExpandLatitude(double latInDegrees, double lonInDegrees, double kilometers)
    {
        ...
        (double MinLatInDegrees, double MinLonInDegrees, double MaxLatInDegrees, double MaxLonInDegrees) = GetOneDegreeLatKilometers(latInDegrees, lonInDegrees);
    ...
    public static (double MinLatInDegrees, double MinLonInDegrees, double MaxLatInDegrees, double MaxLonInDegrees) GetOneDegreeLatKilometers(
        double latInDegrees,
        double lonInDegrees,
    ...
    public static double GetOneDegreeLonKilometers(double latInDegrees)
    {
...
public static class Haversine
{
    ...
    {
        double Lat = GeometricHelper.DegreesToRadians(toLatitude - fromLatitude);
        double Lon = GeometricHelper.DegreesToRadians(toLongitude - fromLongitude);

        double a =
            (Math.Sin(Lat / 2) * Math.Sin(Lat / 2)) +
            (Math.Cos(GeometricHelper.DegreesToRadians(fromLatitude)) * Math.Cos(GeometricHelper.DegreesToRadians(toLatitude)) * Math.Sin(Lon / 2) * Math.Sin(Lon / 2));
        double c = 2 * Math.Asin(Math.Min(1, Math.Sqrt(a)));

        return (double)(GeometricHelper.GetEarthRadiusKilometers(toLatitude) * c);
    }
}
